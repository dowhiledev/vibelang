...
void ast_add_child(ast_node_t *parent, ast_node_t *child) {
  ...
  if (parent->child_count == parent->child_capacity) {
    int new_capacity = parent->child_capacity == 0 ? INITIAL_CAPACITY
                                                   : parent->child_capacity * 2;
    ast_node_t **new_children = (ast_node_t **)realloc(
        parent->children, new_capacity * sizeof(ast_node_t *));
    if (!new_children) {
...
void ast_replace_child(ast_node_t *parent, int index, ast_node_t *new_child) {
  if (!parent || !new_child || index < 0 || index >= parent->child_count)
    return;
...
const char *ast_node_type_name(ast_node_type_t type) {
  static const char *type_names[] = {"Program",
                                     "FunctionDeclaration",
                                     "FunctionBody",
                                     "TypeDeclaration",
                                     "VariableDeclaration",

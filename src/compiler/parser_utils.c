...
extern YY_BUFFER_STATE yy_scan_string(const char*); // Fix: use the correct Flex type
extern void yy_delete_buffer(YY_BUFFER_STATE);      // Fix: specify the correct parameter type
extern ast_node_t* parse_result;

// Only declare parse_string here, don't define it - it's already defined in parser.y

...
static void generate_expression_statement(FILE *out, ast_node_t *node,
                                          int indent);
...
static void collect_variables_in_scope(symbol_scope_t *scope, char ***names,
                                       char ***types, int *count);
static void generate_template_substitution(FILE *out, ast_node_t *node,
                                           symbol_scope_t *func_scope,
                                           int indent);
...
static void generate_standard_headers(FILE *out) {
  ...
  fprintf(out, "extern char* format_prompt(const char* template, char** "
               "var_names, char** var_values, int var_count);\n\n");
}
...
static void generate_template_substitution(FILE *out, ast_node_t *node,
  ...
  for (int i = 0; i < var_count; i++) {
    ...
    if (strstr(var_types[i], "int") || strstr(var_types[i], "Int")) {
      fprintf(out,
              "var_values[%d] = malloc(32); sprintf(var_values[%d], \"%%d\", "
              "%s);\n",
              i, i, var_names[i]);
    } else if (strstr(var_types[i], "float") || strstr(var_types[i], "Float") ||
               strstr(var_types[i], "double")) {
      fprintf(out,
              "var_values[%d] = malloc(32); sprintf(var_values[%d], \"%%g\", "
              "%s);\n",
              i, i, var_names[i]);
  ...
  write_indentation(out, indent);
  fprintf(out, "char* formatted_prompt = format_prompt(prompt_template, "
...
static void generate_var_declaration(FILE *out, ast_node_t *node, int indent) {
  ...
  // Determine C type
  const char *type_str =
      type_node ? get_c_type_for_vibe_type(type_node) : "auto";

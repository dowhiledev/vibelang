...
static void test_meaning_types() {
    const char* source = 
        ...
        "    let result2 = convert_temp(normal_int);  // Valid: Int -> Temperature allowed\n"
        "    \n"
        "    let dist: Distance = 100;\n"
        "    let result3 = convert_temp(dist);  // Should pass - Temperature and Distance have compatible base types\n"
        "}\n";
...
__attribute__((weak)) ast_node_t* parse_string(const char* source) {
    printf("Warning: Using mock parse_string function. Some tests may be skipped.\n");
